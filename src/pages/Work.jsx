    ...
    <>
      ...
      <Page title="Work" pageClass="page-work">
        ...
        {createPortal(
          <div
            ref={previewRef}
            className={`projects_floating_preview ${previewOn ? 'is-on' : ''}`}
            aria-hidden="true"
            style={{ '--gifIndex': activeIndex }}
          >
            <div className="projects_preview_track">
              {work.map((w, i) => (
                <div className="projects_preview_slide" key={i}>
                  <img src={w.image} alt="" />
                </div>
              ))}
            </div>
        ...
        <section className="projects_contain u-container">
          <div className="projects_list" role="list" onPointerLeave={onLeaveList}>
            {work.map((w, idx) => (
              <a
                key={idx}
              ...
              >
                ...
                <div className="projects_item_inner">
                  <div className="projects_item_title">{w.title}</div>

                  <p className="projects_item_desc">{w.desc}</p>
                  ...
                  {Array.isArray(w.stack) && w.stack.length > 0 && (
                    <div className="projects_item_stack" aria-label="Stack">
                      {w.stack.slice(0, 6).map((s) => (
                        <span key={s} className="projects_chip">{s}</span>
                      ))}
                    </div>
                  )}
                </div>
        ...
        {modalOpen && activeItem && createPortal(
          <div
